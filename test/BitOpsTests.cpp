...
template <typename T>
class BitOpsTemplateTestsFixture : public testing::Test {
public:
    BitOpsTemplateTestsFixture() = default;
    ~BitOpsTemplateTestsFixture() override = default;
protected:
...
using BitOpsTypes = ::testing::Types<int, unsigned int, long, unsigned long, long long, unsigned long long>;

TYPED_TEST_SUITE(BitOpsTemplateTestsFixture, BitOpsTypes);

TYPED_TEST(BitOpsTemplateTestsFixture, BitOpsCountOneBits) {
    EXPECT_EQ(countOneBits(this->m_countOneBitVal), 2);
}

TYPED_TEST(BitOpsTemplateTestsFixture, BitOpsFirstOneBits) {
    EXPECT_EQ(firstOneBit(this->m_firstOneBitVal), 0);
}

TYPED_TEST(BitOpsTemplateTestsFixture, BitOpsPopFirstOneBits) {
    EXPECT_EQ(popFirstOneBit(&(this->m_firstOneBitVal)), 0);
}